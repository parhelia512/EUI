...
| `scroll` | offset 控制器 | `offset`、`viewport`、`content`、`onChange` | 页面持有 offset 和内容布局 | 本文 |
| `scrollView` | 自动测量滚动容器 | `size`、`offset`、`content`、`onChange` | 页面持有 offset，组件测量内容 | 本文、gallery |
...
| `mouseArea` | 输入热区 | `size`、`onTap`、`onDrag`、`onScroll`、`onContextMenu` | 页面处理交互状态 | 本文，carousel 内部使用 |
...
常用 API：`position`、`size`、`zIndex` / `z`、`radius` / `rounding`、`color`、`cursor`、`disabled` / `enabled`、`scrollStep`、`maxScrollStep`、`dragThreshold`、`suppressClickAfterDrag`、`onTap` / `onClick`、`onPress`、`onRelease`、`onHover` / `onHoverChanged`、`onEnter`、`onLeave`、`onDragStart`、`onDrag`、`onDragEnd`、`onScroll` / `onWheel`、`onContextMenu`。
