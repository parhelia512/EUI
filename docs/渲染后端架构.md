...
- Vulkan primitive 已开始接入真实绘制；text/image 仍是 no-op 链接占位，稳定 UI 绘制必须继续按阶段 5 拆 CPU/GPU 资源。
...
- 图片解码、GIF 帧、远程缓存可以被 OpenGL/Vulkan 共用。
...
- text font resolving、shaping、metrics 可以被 OpenGL/Vulkan 共用。
...
- 不接 primitive/text/image，直到 frame lifecycle 稳定。
...
- `primitive_parity` 示例在 Vulkan/OpenGL 下截图对比无明显视觉差异后，才进入 text/image。

当前状态：

- 已接入预编译 SPIR-V primitive pipeline，不依赖构建期 shader compiler。
...
- 已新增 `examples/primitive_parity.cpp` 作为 primitive-only 视觉验收入口，避免 gallery 的 text/image/no-op 状态干扰 primitive 对比。
...
- 未完成 Vulkan backdrop blur 和截图对比验收，不能标记为视觉等价完成。
